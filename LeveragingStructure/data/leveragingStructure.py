...
class HuggingfaceDataset(object):
    ...
    def getData(*args,**kwargs):
        ...
        dataset = load_dataset(dsname, "all_languages",
                      cache_dir=kwargs.get("cache_dir","/scratch/zeiberg.d/leveragingStructureFinalExperiments/huggingface/"))
        tr = dataset["train"].to_pandas()
        try:
            trainEncodings = torch.load(f"/scratch/zeiberg.d/leveragingStructureFinalExperiments/{dsname}/trainEncodings.pt")
        except FileNotFoundError:
        ...
        if kwargs.get("pca",False):
            try:
                with open(f"/scratch/zeiberg.d/leveragingStructureFinalExperiments/{dsname}/pca.skl.pkl","rb") as f:
                    pca = pickle.load(f)    
            except FileNotFoundError:
                pca = PCA(n_components=25)
                pca.fit(X)
                with open(f"/scratch/zeiberg.d/leveragingStructureFinalExperiments/{dsname}/pca.skl.pkl","wb") as f:
                    pickle.dump(pca,f)
            X = pca.transform(X)
        return X,y,group
